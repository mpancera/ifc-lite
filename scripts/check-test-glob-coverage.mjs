...
function resolveMatched(pkgDir, testScript, testLooking) {
  ...
  // Quoted recursive globs handed to `node --test`, which expands them itself
  // ("src/**/*.test.ts" "src/**/*.test.tsx"). The viewer's script takes this
  // shape rather than a `$(find …)` substitution so it also starts on a
  // Windows checkout, where cmd.exe cannot run the substitution. Each glob is
  // recursive; the matched set is the union.
  const quotedGlobs = [...trimmed.matchAll(/"([^"\s]+\*\*[^"\s]*)"/g)].map((m) => m[1]);
  if (quotedGlobs.length > 0) {
    const regexes = quotedGlobs.map((g) => globToRegExp(g));
    return testLooking.filter((f) => regexes.some((re) => re.test(f)));
  }
