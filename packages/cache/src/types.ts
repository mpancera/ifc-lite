 ...
 */
/**
 * v18→v19 (fork): the curated IFC4.3 class catalogue in `IfcTypeEnum` was
 * renumbered where upstream's IfcMarinePart / IfcFacilityPartCommon landed on
 * the same ids (IfcActor, IfcActuator). A v18 cache from either numbering
 * would restore those entities under the wrong class, so it is re-parsed.
 */
