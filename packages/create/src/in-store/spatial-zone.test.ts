...
import { addSpatialZoneToStore } from './spatial-zone.js';
...
describe('addSpatialZoneToStore', () => {
  it('writes the IfcSpatialZone attribute order', () => {
    const { editor, view } = makeEditor();
    const { spatialZoneId } = addSpatialZoneToStore(editor, anchor, {
  ...
  it('is NOT aggregated into the storey', () => {
    ...
    // made a part of it.
    const { editor, view } = makeEditor();
    addSpatialZoneToStore(editor, anchor, {
      Name: 'BA-01', Position: [0, 0, 0], Width: 10, Depth: 8, Height: 3,
    });
  ...
  it('emits $ when the model has no owner history', () => {
    const { editor, view } = makeEditor();
    const { spatialZoneId } = addSpatialZoneToStore(
      editor,
  ...
  it('rejects a degenerate footprint instead of writing an unusable body', () => {
    ...
    expect(() => addSpatialZoneToStore(editor, anchor, {
