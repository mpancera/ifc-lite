...
import { heightSystemPayload, heightsFileName, serializeHeightSystem } from './serialize.js';
...
describe('heightsFileName', () => {
  it('is the name the reading side looks for', () => {
    // A contract, not a proposal. An earlier version named the file after the
    // source model — `ARC-01.heights.json` — which no reader was
    // looking for and which would therefore never have been picked up.
    assert.equal(heightsFileName(), 'dc.heights.json');
  });

  it('does not depend on which model the system was read from', () => {
    // One height system per PROJECT. Every discipline model is compared
    // against it, so a name taken from whichever model it came out of would
    // be wrong even when it looked helpful.
    assert.equal(heightsFileName(), heightsFileName());
  });
});
